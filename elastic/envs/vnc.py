...
def create_atari_env_deepmind(env_id, mode, **kwargs):
    env = gym.make(env_id)
    env = wrap_deepmind(env, mode, **kwargs)
    env = Vectorize(env)
    env = DiagnosticsInfo(env, mode)
    env = Unvectorize(env)
    return env


def wrap_diagnostics(env, mode):
    env = Vectorize(env)
    env = DiagnosticsInfo(env, mode)
    env = Unvectorize(env)
    return env
...
def create_atari_env_simple(env_id, mode, **kwargs):
...
def DiagnosticsInfo(env, *args, **kwargs):
    return vectorized.VectorizeFilter(env, _DiagnosticsInfo, *args, **kwargs)


class _DiagnosticsInfo(vectorized.Filter):
    def __init__(self, mode, log_interval=500):
        super(_DiagnosticsInfo, self).__init__()
    ...
    def _after_step(self, observation, reward, done, info):
        ...
        if done:
            ...
            if self._mode == 'train':
                ...
                prefix = 'eval'
            to_log[prefix + "/episode_reward"] = self._episode_reward
            to_log[prefix + "/episode_length"] = self._episode_length
            to_log[prefix + "/episode_time"] = total_time
            to_log[prefix + "/reward_per_time"] = self._episode_reward / total_time
            self._episode_reward = 0
            self._episode_length = 0
            self._all_rewards = []
...
class _DiagnosticsInfoVNC(vectorized.Filter):
    ...
    def _after_step(self, observation, reward, done, info):
        ...
        if done:
            logger.info('Episode terminating: episode_reward=%s episode_length=%s', self._episode_reward, self._episode_length)
            total_time = time.time() - self._episode_time
            to_log["training/episode_reward"] = self._episode_reward
            to_log["training/episode_length"] = self._episode_length
            to_log["training/episode_time"] = total_time
            to_log["training/reward_per_time"] = self._episode_reward / total_time
            self._episode_reward = 0
            self._episode_length = 0
            self._all_rewards = []
